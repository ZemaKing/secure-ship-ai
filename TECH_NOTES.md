...
**Why it exists:** before Chunk F, `ChatWindow.tsx` held `sessionId` in its own `useState` and never touched `state`/`event`/`escalation` at all — those fields existed on the wire since Chunk A/E but nothing on the frontend read them. Extracting the read/write logic into one hook means the upcoming code modal (Chunk G, reacts to `event === "code_sent"`) and escalation banner (Chunk H, reacts to `escalation`) can both consume the same hook from `ChatWindow` without either of them needing to know how `sessionId` threading works internally.

**Verified:** `tsc -b`, `vite build`, and `oxlint` all clean after wiring it into `ChatWindow.tsx`. See that section below for the live session-isolation check, since the hook itself has no behavior to test in isolation — its correctness is entirely "does `ChatWindow` behave the same as before, with two truly independent sessions."
...
| `ChatWindow.tsx` | Owns `messages: ChatMessageData[]` (`useState`, seeded with one hardcoded bot message) and `draft: string` (the input) directly. As of Week 2, **Chunk F**, `sessionId`/`state`/`event`/`escalation` no longer live in this component's own `useState` calls — they're read from `const { sessionId, applyResponse } = useChatSession()` (see the hook's own section above). `handleSubmit` optimistically appends the user's message, then calls the generated `useChat()` mutation with `{ data: { message: text, session_id: sessionId } }`; on a `200` response, `onSuccess` calls `applyResponse(response.data)` (previously a direct `setSessionId(...)` call) before appending the reply bubble, so every subsequent turn sends the session id the backend just confirmed. `chatMutation.isPending` drives a "Typing…" bubble and disables the input/send button while a request is in flight. A `messageListRef` + `useEffect` keyed on `[messages, chatMutation.isPending]` scrolls the list to the bottom on every change — without it, new messages/the typing indicator render below the fold since `.chat-window__message-list` is a fixed-height `overflow-y: auto` region that doesn't auto-scroll on its own |
