...
- [x] Chunk E — escalation theater backend: `services/escalation.py`'s `wants_escalation()` (keyword check, evaluated before any Ollama call); `routes/chat.py`'s `_handle_escalation()` short-circuits at the top of the turn, builds the 3 scripted §6.2b text lines + a personalized/generic greeting, sets `EscalatedToHuman`, returns `event="escalated"` with a real `EscalationPayload`. `first_name` resolved only from already-collected/confirmed identity, never the triggering message (Epic G4). Found live: an unverified visitor's post-escalation shipment question wasn't leaking data (no tool exists for that state) but sounded like it had access — fixed with a `build_system_prompt(unverified_escalation=...)` addendum. Verified end-to-end against the real model.
- [x] Cutover — Orval regenerated against the now-final `ChatResponse`/`VerifyCodeResponse` shapes: `EscalationPayload` in `src/api/generated/secure-ship.ts` picked up Chunk E's real `lines`/`agent_name`/`first_name` fields (replacing Chunk A's `human_name`/`greeting` placeholder); `useChat`/`useVerifyCode` hooks unchanged since Chunk D already regenerated those. `npm run build` clean.
- [x] Chunk F — frontend session plumbing: new `src/hooks/useChatSession.ts` holds `sessionId`/`state`/`event`/`escalation` (a small hook, not a global store — no context/reducer, per the project's React-Query-only state convention); `ChatWindow.tsx` now reads/writes all four through it instead of its own `useState`(s). Behavior-preserving refactor, not a rewrite. `tsc -b`/`vite build`/`oxlint` clean; backend-level sanity check confirmed two independent no-`session_id` requests still get distinct UUIDs. User manually confirmed live in two real browser tabs (two different seeded customers) — fully isolated sessions, no cross-tab bleed, "New Chat" issues a genuinely fresh `session_id`.
- [x] Chunk G — `CodeModal` component: new `src/components/CodeModal/` (6-digit input, calls `useVerifyCode()`, shows `attempts_remaining` inline, Escape/backdrop-dismissible), styled against `ai-chatbot-modal-mockup.png`. Built as `CodeModal.scss` (plain BEM) rather than the spec's `.module.scss`, per the locked no-CSS-Modules decision. `ChatWindow.tsx` renders it only when `event === "code_sent"`, using a remounting `key` (`codeModalKey`) to force a clean reset on every genuinely new code — including a post-lockout resend, where the `event` string value is unchanged and wouldn't otherwise be seen as "new." `tsc -b`/`vite build`/`oxlint` clean; user manually confirmed live (no browser-automation tool available this session): Escape-dismiss works cleanly, and a full lockout → follow-up message → auto-resend sequence correctly reopens the modal fresh.
- [x] Chunk H — `EscalationBanner` component + color shift: new `src/components/EscalationBanner/` renders `escalation.lines` with a client-side staggered reveal (700ms apart) — all cosmetic pacing over data the backend already sent in one response, per Epic G3. Fires an `onHumanJoined` callback once, keyed off a fixed line-index assumption (index 1, "Melany has entered the chat" — the stable position in `ESCALATION_SCRIPT_LINES`), which `ChatWindow.tsx` uses to toggle a `chat-window--human-joined` class for the mint color shift. `ChatMessageData`/`ChatRole` gained an `escalation` variant (same precedent as `shipment?`); `ChatMessage.tsx` renders the banner full-width instead of a normal bubble for that role. Built as `EscalationBanner.scss` (plain BEM), not the spec's `.module.scss`, same locked-decision deviation as Chunk G. `tsc -b`/`vite build`/`oxlint` clean; backend contract + Epic G4 (post-escalation, still-unverified shipment question still declines) re-confirmed via direct `curl`; the actual staggered-reveal/color-shift animation was user-confirmed live from both an `Anonymous` session (nameless greeting) and a `Verified` one (personalized greeting, real seeded customer Jovana Markovic).
- [x] Chunk I — tests, hardening, docs: new `backend/tests/` (12 tests, 5 files, `pytest.ini`), run against the real dev Postgres via a transaction-per-test rollback fixture (no second test DB) plus an autouse fixture clearing the in-memory 2FA store between tests. Covers session isolation (Chunk A), identical-rejection-wording across every single-field mismatch (Epic B3), the full 2FA flow — lockout/match/expiry, the last via a monkeypatched clock rather than mutating stored state (Chunk D) — the tool allowlist rejecting both a hallucinated name and a real tool once the state stops offering it (Epic F2), and escalation-then-shipment-question staying gated while unverified with `ollama_client.chat` mocked so the real, slow model is never needed for this assertion (Epic G4). `pytest backend/tests`: 12/12 green. Also manually re-walked the happy path, a rejected-then-retried identity, and a locked-out-then-restarted code against the real running stack (real `qwen3:8b`, real seeded customers) — all three behaved per §6.2.
- [x] Frontend test suite (follow-up to Chunk I, not its own lettered chunk): **Vitest** (reuses `vite.config.ts` directly, no separate transform config) + Testing Library. `useChatSession.test.ts` (4 tests) and `CodeModal.test.tsx` (6 tests) — deliberately scoped to the two places with real branching logic, not `ChatWindow`'s layout. Only `global.fetch` is mocked in `CodeModal.test.tsx`; React Query and the component's own state machine run for real, mirroring Chunk I's "mock only the true external boundary" approach. `npm test`: 10/10 green; `npm run build`/`npm run lint` stay clean with the test files present. Standing instruction going forward: keep both suites updated as covered code changes, not just on request.
