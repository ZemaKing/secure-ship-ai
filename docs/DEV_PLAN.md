...
- [ ] **Chunk A — Auth0 skeleton.** Create the Auth0 tenant + a **SPA** application + a separate **API** resource (audience) in the dashboard first — before invoking the skill, not mid-way. Install Auth0 Agent Skills (`npx skills add auth0/agent-skills`). Prompt naturally: *"Add Auth0 login to the admin panel in my React frontend and protect the `/admin/*` routes in my FastAPI backend"* — let framework detection pick the SDK guide. Review every generated line (Section 4.5). Backend: `auth/token_verifier.py` (JWKS validation) + `auth/dependencies.py` (`get_current_admin`) + `routes/admin.py` (router-level `Depends`, one stub `GET /admin/me`). Frontend: add `@auth0/auth0-react` + `react-router-dom` (none installed today), `BrowserRouter` + `Auth0Provider` in `main.tsx`, `ProtectedRoute`, minimal `AdminApp` shell, wire `AdminAccessCard`'s dead link to a real route. New `AUTH0_*`/`VITE_AUTH0_*` env vars in `.env` + `docker-compose.yml` (including a new `frontend.environment` block that doesn't exist today). Orval regen #1. Verify live: real Universal Login redirect → real claims from `/admin/me`; `curl` with no token → `401`.
- [ ] **Chunk B — Customer CRUD.** Establishes the pattern the next two days copy: `schemas/admin.py` (Create/Update/Out), `services/admin_customers.py`, thin CRUD routes, 409 (not 500) on deleting a customer with existing shipments. Frontend `admin/CustomerManager/`. Orval regen #2. Verify live: create/edit/delete (including the blocked-delete case) through the real browser UI.
- [ ] **Chunk C — Shipment CRUD + status update.** Same shape as Chunk B, plus a status-dropdown row action (the literal Monday demo gesture). Orval regen #3. Verify live — the real dry run of Monday demo items #2+#3 together: update a shipment's status in the admin UI, then ask a verified chat session about that same shipment and confirm the answer reflects it.
- [ ] **Chunk D — Package CRUD (completes E2) + lightweight Dashboard.** `admin/PackageManager/`. Dashboard stat cards/recent-shipments computed client-side via `useMemo` over data `ShipmentManager` already fetches — no new backend endpoint. Time-permitting if `PackageManager` runs long. Orval regen #4.
- [ ] **Chunk E — Hardening + docs + demo dry run.** `tests/test_admin_chat_separation.py` — the Epic E4 proof: admin routes 401 with no/invalid token; source-inspection assertion that no `services/admin_*.py`/`routes/admin.py` file ever references `ChatSession`; chat/verify routes are provably indifferent to an admin bearer token being present; an admin customer edit never mutates a `ChatSession` row. Standard 4-doc pass (`CHANGE_LOG.md`/`TECH_NOTES.md`/`DEV_PLAN.md`/`CLAUDE.md`) — Monday demo checkboxes stay unchecked until the actual live demo. Capture the Auth0-skill case-study notes (what it got right immediately vs. what needed manual correction) while fresh. Full live dry run of the fixed Monday checklist below, plus one unofficial 4th beat: a stray admin bearer token on a chat/verify call is proven to do nothing.
