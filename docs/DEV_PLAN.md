...
- [x] **Chunk D2 — Lightweight Dashboard.** New `admin/Dashboard/Dashboard.tsx` — 4 stat cards (Total/In Transit/Delivered/Exceptions) + a Recent Shipments mini-table, per `admin-dashboard.png`, computed client-side via `useMemo` over data `ShipmentManager` already fetches (React Query dedupes it — no new backend endpoint). The mockup's "+X% from last month" deltas and System Status/User Management/Quick Actions panels deliberately left out — no backing data or epic for any of them. `/admin/dashboard` wired as a real route, Dashboard nav item enabled (the last one), `/admin`'s index redirect switched to `dashboard`. Found and fixed a real bug along the way: gating on react-query's `isLoading` let the component flash empty content before the real fetch started (since `isLoading` is `false` while a query is merely `disabled`, not yet fetching) — fixed by gating on `data === undefined` instead. Verify live: all 4 stat cards cross-checked against real `psql` counts (55/16/21/2, all matched). `npm test` grew from 29 to 32, no backend changes.
- [x] **Chunk E — Hardening + docs + demo dry run.** Closed the "anyone who signs up is an admin" gap found live in Chunk A: RBAC + "Add Permissions in the Access Token" enabled on the API, `admin:access` permission defined and assigned to the real admin user, public self-service Sign Up disabled on `Username-Password-Authentication`, and `auth/dependencies.py`'s `get_current_admin` now `auth0.require_auth(scopes="admin:access")` (every `/admin/*` route inherits it via the existing router-level dependency). New `tests/test_admin_chat_separation.py` — the Epic E4 proof, 5 assertions: every `/admin/*` route 401s with no/invalid token; source-inspection assertion that no `services/admin_*.py`/`routes/admin.py` file ever references `ChatSession`; `/chat`/`verify-code` behave identically with or without a stray admin bearer token (Ollama mocked); an admin customer edit never mutates a `ChatSession` row. Getting RBAC to actually work live (not just in tests) took three real, stacked fixes beyond the Dashboard steps above: the frontend wasn't requesting `admin:access` as an OAuth *scope* at all (the SDK's `require_auth(scopes=...)` checks the token's `scope` claim, not the RBAC `permissions` array — a real gap between the two); and even after fixing that, the API's "Application Access" tab (per-app authorization policy) still had the SPA at "0/1 permissions granted" until explicitly checked there too. See `CHANGE_LOG.md`'s Chunk E entry for the full live-debugging trail. Full live dry run of the fixed Monday checklist below, plus the unofficial 4th beat (a stray admin bearer token on a chat/verify call does nothing, `curl`-verified) — all passed, including a real, unstaged instance of the token-without-`admin:access` → `403` case, hit organically while debugging the RBAC wiring itself. `pytest backend/tests` grew from 39 to 44; the frontend suite from 32 to 33 (a Chunk-E-adjacent Copy-to-Clipboard addition to `CustomerManager`), both green.
...
- [x] Edge-case pass: expired codes, malformed input, empty states, "give up mid-verification and ask about something else"
...
- [x] **Codegen-suggestion Agent Skill** — `.claude/skills/orval-regen-suggestion/SKILL.md`, a `SKILL.md` that notices backend route/schema files changed and *suggests* (never auto-runs) an Orval regen. Semi-automatic by design — it must wait for a yes. Needed a small `.gitignore` fix alongside it: `.claude/skills/` had been fully ignored (installed for the Auth0 skill), which silently hid this new project-authored skill too; narrowed to ignore only the `auth0` symlink so project skills commit normally.
- [x] **Full Docker Compose (containerize Ollama)** — moved Ollama into a new `ollama/ollama:latest` Compose service, backend reaches it via `ollama:11434` (container-to-container) instead of `host.docker.internal`, with a named `ollama_data` volume so the ~5.2GB `qwen3:8b` pull only happens once. Since this machine has no GPU to lose either way, this was a pure Docker-wiring exercise, not a performance trade-off. One real fix needed along the way: the base image's entrypoint only runs `ollama serve`, so pulling the model needs the server already accepting requests first — a fixed `sleep 5` before the pull was tried first and raced a slow first-boot server (`ollama pull` failed with "could not connect"), fixed by polling `ollama list` in a loop until it succeeds before pulling. Live-verified: a real `POST /chat` round trip against the containerized model, both test suites still green (71 backend/55 frontend) afterward.

**Final Demo (Friday, Week 5 — no Monday follow-up):**
- [x] Full end-to-end walkthrough: anonymous → identity collection → 2FA → verified shipment chat → admin edit reflected live. Browser-driven, live, with every step cross-checked against the real backend (`psql`, `docker compose logs backend`) in parallel rather than trusted on the UI's word alone — see `CHANGE_LOG.md`'s Final Demo entry for the full step-by-step. Also proved, unscripted, that losing the browser session forces a genuine re-verify (no persistent end-user login), and that the admin write and chat read paths share the same live Postgres rows across two *independent* sessions, not a cache.
- [x] Short retro: what Claude Code got right immediately, what needed correction, what you'd do differently with more time.

  **Right immediately, no rework:** the backend-side enforcement design — tool allowlisting per session state, `lookup_shipments` scoping to `session.customer_id` with no identifier parameter ever existing in its signature, and structural separation between Auth0 admin auth and the conversational identity gate. These held from their first draft (Weeks 2–4) through an adversarial pass (Week 3) and a full RBAC hardening pass (Week 4, Chunk E) without needing a redesign — only additive tests as new legitimate cases appeared (e.g. `admin_sessions.py`'s read-only exception in Week 5).

  **Needed real correction:** almost entirely frontend/styling and environment-specific plumbing, not core logic. Recurring bug shapes worth naming: a stale-closure/remount-key issue that hit `CustomerFormModal`, `ShipmentFormModal`, and `PackageFormModal` in turn until the pattern was learned; `Dashboard`'s loading state gated on react-query's `isLoading` instead of `data === undefined`, which flashed empty content because `isLoading` reads `false` while a query is merely `disabled`; `display: flex` directly on a `<td>` silently breaking its border rendering across every admin table, fixed by moving the flex layout to an inner `<div>`. None of these were logic bugs — all were framework/browser behavior that only surfaced by actually running the thing.

  **Infra specifics no amount of code-reading would have caught, only running it:** Auth0's `scope` (OAuth) claim and `permissions` (RBAC) claim are separate, and `auth0-fastapi-api`'s `require_auth(scopes=...)` only checks the former — a real, non-obvious SDK behavior found by reading the SDK's own source, not its docs. `Auth0Provider` needing an *explicit* `redirect_uri` (the SDK's default silently didn't work). `host.docker.internal` vs. a Compose service name for container-to-container traffic. A fixed `sleep 5` before `ollama pull` racing a slow container first boot, fixed by polling instead.

  **What we'd do differently with more time:** write the remount-key and `isLoading`-vs-`undefined` lessons down as an explicit frontend-conventions note *before* the third manager component hit the same bug, not after; and adversarially test the admin RBAC gap (Week 4, Chunk E) *during* Chunk A's build rather than as a dedicated follow-up chunk once it was noticed live.
- [x] Recorded or presented live — either is fine. Presented live, browser-driven by the engineer with Claude Code verifying each step backend-side in parallel (see above).
