...
- [x] Human escalation theater (Epic G / Section 6.2b): "I want to talk to a human" intent recognized from both `Anonymous` and `Verified` states (backend done, Chunk E), plays the scripted sequence (acknowledgment → "X has joined" → personalized greeting if name is known) — color shift is frontend-only, still pending (Chunk H)
