...
- `tests/` (Chunk I, `pytest.ini` sets `pythonpath = .`) — 17 tests across 7 files, run against the real dev Postgres via a transaction-per-test rollback fixture in `conftest.py` (no second test DB, no truncate step) plus an autouse fixture clearing the in-memory 2FA store between tests. Covers session isolation, identical-rejection-wording across every single-field identity mismatch, the full 2FA flow (lockout/match/expiry — expiry via a monkeypatched clock, not a mutated `expires_at`), the tool allowlist rejecting both a hallucinated name and a real tool once a state stops offering it, post-escalation gating while unverified (with `ollama_client.chat` mocked so the real model is never needed for this assertion), `lookup_shipments`'s own scoping (no identifier parameter exists, two sessions get two non-overlapping result sets), an explicit adversarial pass proving a prompt-injection attempt and a smuggled foreign customer-id argument both fail to widen the query (Week 3, Chunk D — see `docs/ADVERSARIAL_TESTING.md` for the demoable write-up, including what the real model did when the injection was tried live), and (post-Week-3 fix, 2026-08-04) that a rejected tool call's reply never falls back to the model's own raw narration, which could otherwise name the rejected tool itself
