...
- `admin/AdminApp.tsx` (+ `.scss`) (Week 4, Chunk A) — the minimal proof-of-concept shell: fetches a real access token via `getAccessTokenSilently()` and calls the generated `useAdminMe()` with it attached as a `Bearer` header (Orval's generated hooks accept a `fetch: RequestInit` override per call — that's the only way a token reaches them, Orval itself has no Auth0 awareness), rendering the real decoded `sub`/`email` claims. Grows into `CustomerManager`/`ShipmentManager`/`PackageManager`/`Dashboard` across Chunks B-D.
- Frontend test suite (follow-up to Chunk I) — **Vitest**, not Jest, since it reuses `vite.config.ts` directly (`vitest.config.ts` = `mergeConfig(viteConfig, {...})`, no second transform/config to maintain) + Testing Library. `hooks/useChatSession.test.ts`, `components/CodeModal/CodeModal.test.tsx`, and (Week 4, Chunk A) `auth/ProtectedRoute.test.tsx` (co-located with what they test), deliberately scoped to the places with real branching logic rather than every component. Only the true external boundary is mocked per file — `global.fetch` for `CodeModal`, `useAuth0()` for `ProtectedRoute` — everything else (React Query, component state machines) runs for real. `npm test`: 17/17 passing
- `orval.config.ts` + `src/api/generated/secure-ship.ts` — Orval installed and configured (`client: 'react-query'`, `httpClient: 'fetch'`, no axios dependency), generated against the backend's live `/openapi.json`; exports `useChat()` (mutation, from `POST /chat`), `useVerifyCode()` (from `POST /verify-code`, Chunk D), and `useAdminMe()` (from `GET /admin/me`, Week 4 Chunk A) — all given explicit backend-side `operation_id`s for clean hook names — plus `ChatRequest`/`ChatResponse` (including `session_id`/`state`/`event`/`escalation`, the last now the real `lines`/`agent_name`/`first_name` shape as of Chunk E's Cutover regeneration), `VerifyCodeRequest`/`VerifyCodeResponse`, and `AdminMeResponse` types. Regenerate manually via `npm run generate:api` whenever backend routes/models change. `QueryClientProvider` wired up in `main.tsx`, inside a new `BrowserRouter` + `Auth0ProviderWithNavigate` (Week 4, Chunk A — neither existed before).
...
- `pytest` — runs `backend/tests/` (20 tests across 8 files); requires the dev Postgres to be up and migrated (`DATABASE_URL` in `.env`) and, as of Week 4 Chunk A, `AUTH0_DOMAIN`/`AUTH0_AUDIENCE` set in `.env` (`auth/dependencies.py` reads them at import time) — but never calls the real Ollama model or a real Auth0 endpoint; each test runs in its own rolled-back transaction, so nothing it writes persists
