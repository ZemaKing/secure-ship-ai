...
Week 2 is in progress — Chunks A–C of a 10-chunk plan are done (see `docs/DEV_PLAN.md`'s Week 2 progress notes for the chunk list, and `CHANGE_LOG.md` for the day-by-day narrative). The chat is no longer ungated: a visitor asking about a shipment gets conversationally asked for their identity, extracted across multiple turns in any order, and matched (or neutrally rejected) against seeded `Customer` records. Still missing: real 2FA send/verify (today's "matched" reply is a placeholder — Chunk D), and escalation theater (Chunk E).
...
- `schemas/chat.py` / `schemas/verify.py` — `ChatRequest`/`ChatResponse` (with `session_id`/`state`/`event`/`escalation` fields) and the not-yet-wired `VerifyCodeRequest`/`VerifyCodeResponse`, pulled out of `routes/` into their own Pydantic modules
...
- `routes/chat.py` orchestration (Chunk C): `_tools_for_state()` offers `verify_identity` only while `Anonymous`/`CollectingIdentity`; `_dispatch_tool()` re-checks the model's tool-call name against that same allowlist (rejects a hallucinated/prompt-injected name, drops unexpected extra arguments) before running anything; `REJECTED` returns a fixed neutral message with no second model call; `PARTIAL` makes a second model call (no tools) to phrase a natural follow-up for the still-missing fields; `MATCHED` is a placeholder reply pending Chunk D. A small keyword check (`_mentions_shipment()`, not a model call) covers the one case the model won't reliably signal itself: a from-scratch session's very first, identity-free message still flips `Anonymous → CollectingIdentity`
...
`backend/` and `frontend/` are both real and running (see "Project state" above); `scripts/` has a working seed script. This is the target shape everything should converge on — `backend/tools/` is the enforcement layer described above, partially built (`verify_identity.py` exists; `send_verification_code.py`/`check_verification_code.py`/`lookup_shipments.py` are Week 2–3 scope, not yet built), and `frontend/src/api/generated/` is Orval output that should never be hand-edited.
