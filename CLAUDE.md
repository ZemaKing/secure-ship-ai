...
- `services/admin_shipments.py` (Week 4, Chunk C) — same plain-function shape as `admin_customers.py`, plus `routes/admin.py`'s five Shipment routes and `_to_shipment_out()` (denormalizes `customer_name` onto the response so the admin table needs no second join). `schemas/admin.py::ShipmentUpdate` is a deliberate deviation from `CustomerUpdate`'s full-replace shape — every field `Optional`, so the admin UI's status-dropdown row action can `PATCH` just `{"status": ...}` (via `update_shipment`'s `data.model_dump(exclude_unset=True)` merge) without resubmitting the rest of the record. `DELETE` gets the same `409`-on-children treatment for a shipment with existing packages
- `services/admin_packages.py` (Week 4, Chunk D1) — same plain-function shape again, plus `routes/admin.py`'s five Package routes and `_to_package_out()` (denormalizes the parent shipment's `tracking_number`). `schemas/admin.py::PackageUpdate` is a plain full-replace shape, like `CustomerUpdate` rather than `ShipmentUpdate` — no row action needs a partial merge here. `DELETE` has no `409`-on-children case — nothing has a foreign key pointing at `packages.id`, the first CRUD entity this week without one
...
- `admin/AdminLayout.tsx` (+ `.scss`) (Week 4, Chunk B — replaces Chunk A's bare `AdminApp.tsx`) — the real admin shell per `admin-pages.png`: a dedicated sidebar (Dashboard/Customers/Shipments/Packages nav, Customers/Shipments/Packages wired as of Chunk D1, only Dashboard left as a non-clickable "Soon" item, each with a real nav icon) plus a header with a working Logout button. Renders a react-router `<Outlet />` for whichever admin page is active. `admin/useAdminAccessToken.ts` (Week 4, Chunk B) — extracted the `getAccessTokenSilently()`/`authHeaders()` dance into a shared hook once `AdminLayout` and `CustomerManager` both needed it; `CustomerManager`, `ShipmentManager`, and (Chunk D1) `PackageManager` are its real consumers now. `admin/ConfirmDialog/` (Week 4, Chunk B) — a generic delete-confirmation modal, built once for reuse across Customer/Shipment/Package delete flows, not per-manager; reused as-is by `ShipmentManager` in Chunk C and `PackageManager` in Chunk D1, no changes needed. `admin/CustomerManager/` (Week 4, Chunk B) — table (Name/Phone/Address/Actions) with client-side search and a two-direction Name-column sort, icon-based Edit/Delete row actions (a `mask-image`/`currentColor` technique to recolor the otherwise-hardcoded-black SVGs), `CustomerFormModal` (Add/Edit, same form per `admin-modals.png`'s "doubles as Edit" note) — its remount `key` (`customer.id`/`'create'`/`'closed'`) is required, not decorative, since the modal stays mounted across Add→Edit and its form-state initializer only runs on true first mount (same class of bug `CodeModal`'s own remount-key trick already solved once, for a different reason). `admin/ShipmentManager/` (Week 4, Chunk C) — table (Tracking Number/Customer/Origin/Destination/Carrier/Status/Est. Delivery/Updated/Actions) with a first-class status-dropdown row action (`PATCH`ing just `{"status": ...}`, the literal Monday demo gesture) doubling as a colored pill via the existing `$color-status-*` tokens, `ShipmentFormModal` (Add/Edit) applying both fixes `CustomerFormModal` learned the hard way (remount `key`, picking only editable fields off the `*Out` prop) from the start. `admin/PackageManager/` (Week 4, Chunk D1) — table (Tracking Number/Description/Weight (kg)/Declared Value/Actions), `PackageFormModal` (Add/Edit, with a Shipment `<select>` populated from `useListShipments()`) applying both fixes from the start too — the third manager in a row to ship bug-free on the first pass. `utils/formatDate.ts` (Week 4, Chunk C) — `formatDateTime`/`formatDate` extracted out of `ChatWindow.tsx` into the frontend's first shared, non-feature-specific utility folder, once `ShipmentManager` needed the same formatting.
- Frontend test suite (follow-up to Chunk I) — **Vitest**, not Jest, since it reuses `vite.config.ts` directly (`vitest.config.ts` = `mergeConfig(viteConfig, {...})`, no second transform/config to maintain) + Testing Library. `hooks/useChatSession.test.ts`, `components/CodeModal/CodeModal.test.tsx`, `auth/ProtectedRoute.test.tsx` (Week 4, Chunk A), `admin/CustomerManager/CustomerManager.test.tsx` (Week 4, Chunk B), `admin/ShipmentManager/ShipmentManager.test.tsx` (Week 4, Chunk C), and `admin/PackageManager/PackageManager.test.tsx` (Week 4, Chunk D1) — all co-located with what they test, deliberately scoped to the places with real branching logic rather than every component. Only the true external boundary is mocked per file — `global.fetch` for `CodeModal`/`CustomerManager`/`ShipmentManager`/`PackageManager`, `useAuth0()` for `ProtectedRoute`/`CustomerManager`/`ShipmentManager`/`PackageManager` — everything else (React Query, component state machines) runs for real. `CustomerManager.test.tsx` caught two real bugs before they reached the browser: the remount-key issue above, and a customer `id` leaking into `PATCH` bodies because the edit form's initial state was seeded directly from the `CustomerOut` prop instead of picking only the editable fields — both fixes applied pre-emptively in `ShipmentFormModal` and `PackageFormModal`, so both passed on their first run (4 tests each). `npm test`: 29/29 passing
- `orval.config.ts` + `src/api/generated/secure-ship.ts` — Orval installed and configured (`client: 'react-query'`, `httpClient: 'fetch'`, no axios dependency), generated against the backend's live `/openapi.json`; exports `useChat()` (mutation, from `POST /chat`), `useVerifyCode()` (from `POST /verify-code`, Chunk D), `useAdminMe()` (from `GET /admin/me`, Week 4 Chunk A), `useListCustomers()`/`useCreateCustomer()`/`useUpdateCustomer()`/`useDeleteCustomer()` (Week 4, Chunk B), `useListShipments()`/`useCreateShipment()`/`useUpdateShipment()`/`useDeleteShipment()` (Week 4, Chunk C), and `useListPackages()`/`useCreatePackage()`/`useUpdatePackage()`/`useDeletePackage()` (Week 4, Chunk D1) — all given explicit backend-side `operation_id`s for clean hook names — plus `ChatRequest`/`ChatResponse` (including `session_id`/`state`/`event`/`escalation`, the last now the real `lines`/`agent_name`/`first_name` shape as of Chunk E's Cutover regeneration), `VerifyCodeRequest`/`VerifyCodeResponse`, `AdminMeResponse`, `CustomerCreate`/`CustomerUpdate`/`CustomerOut`/`ErrorDetail` types (the last a shared 409-conflict body shape, reused for the same delete-with-children pattern in Shipment CRUD), `ShipmentCreate`/`ShipmentUpdate`/`ShipmentOut` (the last including a denormalized `customer_name`; `ShipmentUpdate`'s fields all generated as optional, matching the backend's partial-update schema), and `PackageCreate`/`PackageUpdate`/`PackageOut` (the last including a denormalized `tracking_number`; `weight_kg`/`declared_value` typed `number | string` on the create/update side, a plain pattern-constrained `string` on the way out, same Decimal-via-OpenAPI quirk `ShipmentCard`'s mapper already works around). Regenerate manually via `npm run generate:api` whenever backend routes/models change — Chunk D1's regen needed the already-running Docker backend stopped first, since a stale container was still answering `/openapi.json` with the pre-Chunk-D schema. `QueryClientProvider` wired up in `main.tsx`, inside a new `BrowserRouter` + `Auth0ProviderWithNavigate` (Week 4, Chunk A — neither existed before).
...
- `pytest` — runs `backend/tests/` (39 tests across 11 files); requires the dev Postgres to be up and migrated (`DATABASE_URL` in `.env`) and, as of Week 4 Chunk A, `AUTH0_DOMAIN`/`AUTH0_AUDIENCE` set in `.env` (`auth/dependencies.py` reads them at import time) — but never calls the real Ollama model or a real Auth0 endpoint; each test runs in its own rolled-back transaction, so nothing it writes persists
...
`backend/tests/` (Chunk I onward) now covers session isolation, neutral-rejection wording, the full 2FA flow (lockout/match/expiry), the state-scoped tool allowlist, post-escalation gating while unverified, `lookup_shipments` scoping under adversarial pressure, and admin auth/Customer/Shipment/Package CRUD (including the delete-with-children `409` and the admin-write→chat-read proof) — 39/39 passing. `frontend`'s Vitest suite covers `useChatSession`'s field-merging/normalization, `CodeModal`'s digit-input/lockout state machine, `ProtectedRoute`'s redirect logic, and `CustomerManager`/`ShipmentManager`/`PackageManager`'s CRUD flows — 29/29 passing. Standing instruction: keep both suites updated as covered code changes, and add new tests for new logic, without waiting to be asked each time.
